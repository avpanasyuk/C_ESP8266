...
namespace avp {
   ...
   */
  const char *SendGET(const char *server, const String &Message, uint16_t port = 80, unsigned long Timeout = DefaultTimeout_ms);
  const char *SendGET_Secure(const char *server, const String &Message, uint16_t port = 443, unsigned long Timeout = DefaultTimeout_ms);
